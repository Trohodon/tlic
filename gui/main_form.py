...
class MainForm(ttk.Frame):
    ...
    def _build_left_controls(self, parent: ttk.Frame) -> None:
        ...
        ttk.Radiobutton(selection, text="Summer", value="Summer", variable=self.season_var, command=self.on_season_change).grid(
            row=7, column=0, sticky="w", pady=(4, 0)
        )
        ttk.Radiobutton(selection, text="Winter", value="Winter", variable=self.season_var, command=self.on_season_change).grid(
    ...
    def _refresh_selectors(self) -> None:
        ...
        if self.cond_var.get() not in cond_combo and cond_names:
            self.cond_var.set(cond_names[0])
        elif not self.cond_var.get() and cond_combo:
    ...
    def _apply_season_default_temperature(self) -> None:
        default_temp = 40.0 if self.season_var.get() == "Summer" else 26.7
        self.temp_var.set(f"{default_temp:.1f}")
    ...
    def on_cond_or_season_change(self) -> None:
        ...
        self.cond_desc.insert("end", f"Rating A:\t{cond.rate_a:.0f} A\n")
        self.cond_desc.insert("end", f"Rating B:\t{cond.rate_b:.0f} A\n")
        self.cond_desc.insert("end", f"Rating C:\t{cond.rate_c:.0f} A\n")
