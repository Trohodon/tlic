...
class LineRateMainForm(ttk.Frame):
    def __init__(self, parent) -> None:
        ...
        season_box = ttk.Combobox(form, textvariable=self.var_season, values=["Summer", "Winter"], state="readonly", width=12)
    ...
    def on_season_change(self) -> None:
        self.var_amb.set("40.0" if self.var_season.get() == "Summer" else "26.7")
        self.calculate()
    ...
    def calculate(self) -> None:
        ...
        try:
            ...
            is_summer = self.var_season.get() == "Summer"
            amb = float(self.var_amb.get())
            mot = float(self.var_mot.get())

            self.rating.select_conductor_solve(is_summer, cond, amb, mot)
