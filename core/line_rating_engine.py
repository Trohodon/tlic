...
@dataclass
class ThermalModel:
    # Simplified thermal model used by the Python port.
    diameter: float = 25.4
...
class LineRatingCalc:
    ...
    def get_ratings_from_data(
        ...
        # Return table ratings only when a valid table source exists.
        _ = (conductor_name, season, amb_temp, max_temperature)
        if conductor is None:
            return None
        if not conductor.has_table_ratings:
            return None
        if conductor.rate_a <= 0.0 or conductor.rate_b <= 0.0 or conductor.rate_c <= 0.0:
            return None
        return conductor.rate_a, conductor.rate_b, conductor.rate_c
    ...
    def select_conductor_solve(self, season: str, conductor: Conductor, amb_temp: float, max_temperature: float) -> str:
        # Table ratings first, thermal fallback second.
        table_ratings = self.get_ratings_from_data(conductor.name, season, amb_temp, max_temperature, conductor)
        if table_ratings is not None:
            self.rate_a, self.rate_b, self.rate_c = table_ratings
            return ""
